...
export default function ExpirenceSection({title,children,src,reverse}:ExpirenceSectionProps){
    ...
    return(
            ...
            <section className="p-[1rem] md:p-[2rem] flex items-center flex-col w-[100%] gap-[0.5rem] ">
                ...
                <div className={`flex ${reverseImage[0]} justify-center gap-[1rem] items-center flex-col md:flex-row`}>
                    <Image className="rounded-[0.5rem] w-[16rem] h-[16rem]" alt={title} src={src} width={250} height={250} ></Image>
                    
                    <p className="text-text_color text-center h-[70%] break-words max-w-[80vw] md:max-w-[60vw] text-[1rem] md:text-[1.5rem]">
                        {children}
                    </p>
                </div>
